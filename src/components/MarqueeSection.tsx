...
export function MarqueeSection() {
  ...
  const row1Images = [...ROW1_IMAGES, ...ROW1_IMAGES, ...ROW1_IMAGES];
  const row2Images = [...ROW2_IMAGES, ...ROW2_IMAGES, ...ROW2_IMAGES];
    ...
    >
      ...
      >
        {row1Images.map((src, index) => (
          <ImageTile key={`row1-${index}`} src={src} />
        ))}
      </div>
      ...
      >
        {row2Images.map((src, index) => (
          <ImageTile key={`row2-${index}`} src={src} />
        ))}
      </div>
    </section>
  );
